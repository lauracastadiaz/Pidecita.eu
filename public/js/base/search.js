...
class Search {
  ...
  _init() {
    ...
    const searchResults = document.getElementById(this.settings.searchResultsId);
    // Función para filtrar los resultados
    const filterResults = () => {
      const searchTerm = searchInput.value.trim().toLowerCase();
      const results = searchResults.querySelectorAll('.auto-complete-result-item');

      results.forEach(result => {
          const label = result.querySelector('.mb-0').textContent.toLowerCase();
          const url = result.querySelector('.text-muted.mb-0').textContent.toLowerCase();
          if (label.includes(searchTerm) || url.includes(searchTerm)) {
    ...
    // Añadir evento de entrada en el campo de búsqueda
